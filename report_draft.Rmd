...
`Write summary of  this graph (Vishal)`
...
`Speed of Wind vs Number of Sessions`
...
We began our analysis by establishing a reliable measure of instructor efficiency. As both the number of exercises completed and the number of hours each instructor works are continuous variables, we chose to establish an efficiency mmetric based on the correlation between these two variables.
...
We found that the Pearson correlation coefficient was approximately `r {coef}` indicating a very strong correlation. Based on this, we concluded that the number of exercises students completed under each instructor per hour of work is a strong measure of efficiency. Thus, we generated the second graph above to determine a roughly ideal distribution of hours for each instructor. Instructors whose points lie above the line are being giiven too many hours relative to average performance and instructors below the line are being giiven too few hours relative to their efficiency.
...
```
The above table displays the implied order of exercises based on the session at which 70% of students had completed them at least once. This order seems to suggest that while exercises 1 to 5 are essentially mandatory, later exercises are more or less optional as through 50 sessions, excerises not listed in the table such as 12, 13, 14 and 15 were not completed by at least 70% of students.

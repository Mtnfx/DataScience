...
# Note that when calculating the instructor's yearly hours, I divide by 365.25. This is to account for leap years. I consider this more than accurate enough as, at most, the values will be off by 0.1%.
...
#Coeftext2 = paste("r =" , toString(cor(Instructor_Data %>% select(Average_Hours), Instructor_Data %>% select(Hourly_Exercises), method = c("pearson"))[1][1]))
#ggplot(data = Instructor_Data, aes(x = Total_Hours, y = Total_Exercises)) + geom_point()  +
 # geom_smooth(method='lm', formula= y~x, se = FALSE)+  geom_text(aes(x = 250, y = 300), label = Coeftext) +
  #ggtitle("Hours of Instruction by Instructors vs Total Exercises Students Completed") + ggsave("img/Exercises_Vs_Duration.png")

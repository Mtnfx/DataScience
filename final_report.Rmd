...
seasonal_air_movements %>%
  ...
  mutate(Month = month(Month, label = T))%>%
  ggplot(aes(x = Month, y = VALUE)) + geom_col(aes(fill = movement_type)) + theme(legend.position="bottom", legend.direction="horizontal",
                   legend.title = element_blank()) + ggtitle("Month vs Air Traffic")
```
...
The boxplot suggests that there is a significant difference in the number of exercises completed between both the groups. Hence, we attempted to find the collection of exercises that were typically completed before a student flew solo and graphed the average count of each exercise completed at least once to report our findings. Additionally, we discovered that each instructors qualifying collection differed from others' with instructors 3 and 11 requiring lesser frequencies of exercises 16,17,18,30 than the average and 4,6,7,9 requiring higher. Moreover, instructors 10 and 14 do not have a distinguishing qualifying collection with proportional changes in frequencies between -1 and 1 (see note 3 in appendix for correspoding tables).
...
We have analyzed how close each student has come to completing their pilot license requirement, based on each requiremennt needed to graduate (for exact requirements, see note 1 in appendix).

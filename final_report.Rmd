...
The boxplot suggests that there is a significant difference in the number of exercises completed between both the groups. Hence, we attempted to find the collection of exercises that were typically completed before a student flew solo and graphed the average count of each exercise completed at least once to report our findings. Additionally, we discovered that each instructor's qualifying collection of exercises to fly solo differed from others' with instructors 3 and 11 requiring lesser frequencies of exercises 16,17,18,30 than the average and 4,6,7,9 requiring higher. Moreover, instructors 10 and 14 do not have a distinguishing qualifying collection with exercise counts for both groups of students, those who flew solo and those who did not, are very close to each other (see note 3 in appendix for corresponding tables).
...
We have analyzed how close each student has come to completing their pilot license requirement, based on each requirement needed to graduate (for exact requirements, see note 1 in appendix).
...
## Appendix
1. the exact training time requirements to complete the flight course are listed as follows:
...
2. We conducted a hypothesis test to validate our conclusion from the boxplot of mean exercises completed between students that flew solo and those that did not at 5% significance level.
...
kable(avg_ex_solo_per_ins, 
      caption = "Instructor vs Avg Exercise Count before first solo flight") %>% 
  kable_styling(latex_options = "hold_position")

kable(prop_per_ins, 
      caption = "Instructor vs Proportional change in avg Exercise Count") %>% 
  kable_styling(latex_options = "hold_position")
```

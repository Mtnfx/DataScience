...
seasonal_air_movements %>%
  ...
  mutate(Month = month(Month, label = T))%>%
  ggplot(aes(x = Month, y = VALUE)) + geom_col(aes(fill = movement_type)) + ggtitle("Month vs Air Traffic")
```
...
ggplot(data = Instructor_Data, mapping = aes(x = Average_Hours, y = Hourly_Exercises)) + geom_point() + geom_text(aes(x = Average__Yearly_Hours, y = Hourly_Exercises, label = Instructor_ID, hjust = 0), nudge_x = 1) + geom_smooth(method = 'lm', formula = y~x, se = FALSE) + ggtitle("Hours Instructed per Year vs Hourly Exercises Completed")
```

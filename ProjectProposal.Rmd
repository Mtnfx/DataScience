...
Link to the share RStudio Cloud project that created this report: 'needs to be added'
...
With the data given and additional data on monthly air traffic, weather and seasons, the following questions will be adressed by our analysis:
  ...
  two sessions, the density of student enrollment every month, student performance per season. 
...
air_movements <- read_csv("data/aircraft-movements/23100003.csv") %>% 
...
- `Min Temp (°C)`
- `Mean Temp (°C)`
- `Total Precip (mm)`: Total Precipitation in milimeters during the day
- `Spd of Max Gust (km/h)`: Speed of Maximum Gust in km/h during the day
...
We graphed seasons against the efficiency of student 3.
``` {r, echo=FALSE, warnings=FALSE} 
# Graphing Seasons against Efficiency of Student 3.
clean_data %>% inner_join(seasons_four, by = c("Month")) %>% mutate(numEx = str_count(Exercises, ",") + 1, Efficiency = numEx/Duration) %>% 
  filter(Student_ID==3) %>% group_by(Season) %>% summarise(Efficiency = mean(Efficiency, na.rm = T)) %>% ggplot(aes(x=Season, y=Efficiency)) + 
  geom_bar(stat="identity") + ggtitle("Seasons vs Efficiency Of Student 3") + theme(plot.title = element_text(hjust = 0.5))
```
From this graph, we can infer that students' performance differed from season to season.
...
To establish that flying solo can be considered to be a metric for students progress we found the mean number of exercises completed by students that flew solo including the exercises performed during first solo flight and also the mean number of exercises completed by students that never flew solo.
``` {r, echo=FALSE, warnings=FALSE}
...
clean_data %>% unite("Date", c(Year, Month, Day), sep = "-") %>% 
  ...
  select(Student_ID, totalEx) %>% 
  anti_join(c1 %>% unite("Date", c(Year, Month, Day), sep = "-") %>% 
